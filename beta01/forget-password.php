<?php	
	...
	$form_array 	= array();
	$errorMsg = 'no';
	if($_POST['securityKey']==md5(FORGETPASSWORD)){		
		if($_POST['txtUserEmail'] == ''){
			$form_array['txtUserEmail'] = '<span style=\"font-size:12px; color:#FF0000; font-weight:normal;\">Please enter a valid email address</span>';
			$errorMsg	= 'yes';
		}
		if($errorMsg == 'no' && $errorMsg != 'yes' ) {
			$txtUserEmail 	= $_POST['txtUserEmail'];
			if($usersObj->fun_checkEmailAddress($txtUserEmail, '') === true) {
				$userLoginPass = $usersObj->fun_setUserNewPasswordByEmail($txtUserEmail);
				$frmsubmit 	= "success";
			} else {
				$frmsubmit 	= "notfound";
			}
...
<div id="wrapper">
    ...
    <div id="pg-wrapper" align="center"><h1 class="page-heading"><?php echo tranText('Forgot_your_password'); ?>?</h1></div>
    <div id="main">
        <table border="0" cellspacing="0" cellpadding="0">
            <tr>
                ...
                <td align="left" valign="top" class="font12" style="padding-left:10px;">
					<?php require_once(SITE_INCLUDES_PATH.'forgetpassword.php'); ?>
                </td>
            </tr>
        </table>
    </div>
