...
for($i =0; $i < count($propListArr); $i++) {
	...
	if($propLocInfoArr['area_name'] !=""){
		$propLoc .= "<a href=\"".SITE_URL."vacation-rentals/in.".str_replace(" ", "-", strtolower($propLocInfoArr['area_name']))."\">".ucwords($propLocInfoArr['area_name'])."</a> > ";
	}
	if($propLocInfoArr['region_name'] !=""){
		$propLoc .= "<a href=\"".SITE_URL."vacation-rentals/in.".str_replace(" ", "-", strtolower($propLocInfoArr['region_name']))."\">".ucwords($propLocInfoArr['region_name'])."</a> > ";
	}
	if($propLocInfoArr['subregion_name'] !=""){
		$propLoc .= "<a href=\"".SITE_URL."vacation-rentals/in.".str_replace(" ", "-", strtolower($propLocInfoArr['subregion_name']))."\">".ucwords($propLocInfoArr['subregion_name'])."</a> > ";
	}
	if($propLocInfoArr['location_name'] !=""){
		$propLoc .= "<a href=\"".SITE_URL."vacation-rentals/in.".str_replace(" ", "-", strtolower($propLocInfoArr['location_name']))."\">".ucwords($propLocInfoArr['location_name'])."</a> > ";
	}
	...
	if(is_array($propPriceInfoArr) && (count($propPriceInfoArr) > 0)){
		...
		$show_price 	= "<span id=\"price_currency_symbol_id1".$i."\">".$currency_symbol."</span><span id=\"price_currency_price_id1".$i."\" >".$min_price."</span>-<span id=\"price_currency_symbol_id2".$i."\" >".$currency_symbol."</span><span id=\"price_currency_price_id2".$i."\">".$max_price."</span> p/w <br>";
	} else {
		$show_price 	= "";
	}
...
?>
<input name="<?php echo "price_rate_usd_1".$i;?>"  id="<?php echo "price_rate_usd_id1".$i;?>" type="hidden" value="<?php echo (($min_price/$currencyRateArr[$property_currency_code])*$currencyRateArr['USD']); ?>" />
<input name="<?php echo "price_rate_usd_2".$i;?>"  id="<?php echo "price_rate_usd_id2".$i;?>" type="hidden" value="<?php echo (($max_price/$currencyRateArr[$property_currency_code])*$currencyRateArr['USD']); ?>" />

<input name="<?php echo "price_rate_gbp_1".$i;?>"  id="<?php echo "price_rate_gbp_id1".$i;?>" type="hidden" value="<?php echo (($min_price/$currencyRateArr[$property_currency_code])*$currencyRateArr['GBP']); ?>" />
<input name="<?php echo "price_rate_gbp_2".$i;?>"  id="<?php echo "price_rate_gbp_id2".$i;?>" type="hidden" value="<?php echo (($max_price/$currencyRateArr[$property_currency_code])*$currencyRateArr['GBP']); ?>" />

<input name="<?php echo "price_rate_eur_1".$i;?>"  id="<?php echo "price_rate_eur_id1".$i;?>" type="hidden" value="<?php echo (($min_price/$currencyRateArr[$property_currency_code])*$currencyRateArr['EUR']); ?>" />
<input name="<?php echo "price_rate_eur_2".$i;?>"  id="<?php echo "price_rate_eur_id2".$i;?>" type="hidden" value="<?php echo (($max_price/$currencyRateArr[$property_currency_code])*$currencyRateArr['EUR']); ?>" />
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="listingTable">
    <tr><td colspan="2" align="left" valign="top" class="pad-top5"><p class="smallFont11">Location: <?php echo $propLoc; ?></p></td></tr>
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="listingTable">
                <tr>
                    ...
                    <td width="42%" valign="top" class="pad-lft15 pad-rgt10">
                        ...
                        <p><?php echo $description." ... <a href=\"".$property_link."\">read more</a>"; ?></p>
                    ...
                    <td width="23%" valign="top" class="pad-lft15 pad-top5">
                        <table width="100%" border="0" cellspacing="0" cellpadding="1">
							<?php /*?>
                            <tr><td class="lateDealFavourites"id="favourite<?php echo $i;?>"><?php if($propFavArr){?><a href="javascript:removeFavourite('<?php echo $propFavArr;?>', '<?php echo $property_id;?>', '<?php echo $_SESSION['ses_user_id'];?>', '<?php echo $i;?>')">Remove favourite</a><?php }else{if(isset($_SESSION['ses_user_id']) && $_SESSION['ses_user_id'] != ""){?><a href="javascript:addFavourite('<?php echo $property_id;?>', '<?php echo $_SESSION['ses_user_id'];?>', '<?php echo $i;?>')"><?php }else{?><a href="javascript:toggleLayer1('signup');"><?php }?>Add to favourites</a><?php }?></td></tr>
							<?php */?>
                            ...
                            <tr>
                                ...
                                <?php //echo "<br /><input name=\"txtPropertyCheck".$i."\" id=\"txtPropertyCheckId".$i."\" type=\"checkbox\" value=\"".$property_id."\" onclick=\"chkAddToEnquiry(".$i.");\" checked=\"checked\" />&nbsp;Contact owner"; ?>
                                </td>
                            </tr>
                        </table>
                        <div><a href="<?php echo $property_link; ?>"><img src="<?php echo SITE_IMAGES;?>view-property-btn.jpg" alt="View" /></a></div>
                    </td>
                </tr>
            </table>
        </td>
