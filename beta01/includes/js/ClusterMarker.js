...
ClusterMarker.prototype.addPropertyMarkers=function($markers, $markerHTML){
	var i;
	if(!$markers[0]){
		//	assume $markers is an associative array and convert to a numerically indexed array
		var $numArray=[];
	...
	this.clusterMarkerTitle=$markerHTML;
	this._mapMarkers=this._mapMarkers.concat($markers);
};
...
ClusterMarker.prototype.clusterMarkerClick=function($args){
	...
	var point = $args.clusterMarker.getLatLng();
	var str = String(point);
	var distBody = str.split(", ");
	var latVal = str_replace("(", "", distBody[0]);
	var lonVal = str_replace(")", "", distBody[1]);
	document.getElementById("p_map_latitude").value = latVal;
	document.getElementById("p_map_longitude").value = lonVal;
	document.getElementById("p_map_zoom").value = this._map.getZoom();
};
...
ClusterMarker.prototype.fitMapToMarkers=function(){
	...
	this.refresh();
	var point = $markersBounds.getCenter();
	var str = String(point);
	var distBody = str.split(", ");
	var latVal = str_replace("(", "", distBody[0]);
	var lonVal = str_replace(")", "", distBody[1]);
