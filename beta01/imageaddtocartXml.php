...
if(isset($_GET['pid']) && $_GET['pid'] !=""){
	...
	// Step II: Select Extra images cost
	$products_price = $dbObj->getField(TABLE_PRODUCTS, "products_id", "1", "products_price");
	if($products_price > 0) {
		$total_price = ($add_photos*$products_price);
	} else {
		$total_price = ($add_photos*5).".0000";
	}
	...
	if(($add_photos >0) && $property_id !="" && $user_id !=""){
		...
		$strInsertQuery = "INSERT INTO ".TABLE_USER_CART."(user_basket_id, user_id, products_id, property_id, user_basket_quantity, final_price, user_basket_date_added, user_basket_date_expire) VALUES (null, '".$user_id."', 1, '".$property_id."', '".$add_photos."', '".$total_price."', '".$user_basket_date_added."', '".$user_basket_date_expire."')";
