...
if(isset($_REQUEST['securityKey'])){
	if($_REQUEST['securityKey'] == md5("Add")){
		...
		if(move_uploaded_file($_FILES['img_banner']['tmp_name'], $uploadphotofile)){
			...
			$banner_id = $bannerObj->fun_process_newBanner($_POST);
			if($banner_id <> ''){
				$bannerObj->fun_update_newBanner($banner_id, $photo_main);
			}
		} else {
			$message = "Error: We are unable to update your property photos detail!";
		}

		echo "<script>location.href='admin-site-variables.php?sec=banner'</script>";
		...
		if($_POST['image_action'] == 2){
			...
			$bannerObj->fun_update_newBannerById($_POST, $photo_main);
...
echo "<b>".$addtitle."</b>";
?>
<form name="frm_update_banner" id="frm_update_banner" action="admin-site-variables.php?sec=banneradd" method="post" enctype="multipart/form-data">
										...
										<table width="100%" border="0" cellspacing="0" cellpadding="10" class="eventForm">
											...
											<tr>
												...
												<select name="banner_type" id="banner_type" style="display:block;" class="select216">
                                                    ...
                                                    <option value="2" <?php if($banner_info[0]['banner_type'] == "2") { echo " selected=\"selected\"";} ?>>Home Right Banner</option>
                                                    <option value="3" <?php if($banner_info[0]['banner_type'] == "3") { echo " selected=\"selected\"";} ?>>Search List Banner</option>
                                                    <option value="4" <?php if($banner_info[0]['banner_type'] == "4") { echo " selected=\"selected\"";} ?>>Owner Home Banner</option>
												</select>
                                                </td>
											</tr>
											<tr>
