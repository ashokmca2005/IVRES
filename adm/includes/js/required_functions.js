...
function buildOption(ids, startVal, endVal, selVal){
	var ids = document.getElementById(ids);
	var cnt = 1;
	startVal = parseInt(startVal);
	endVal = parseInt(endVal);
	selVal = parseInt(selVal);
	ids.options[0] = new Option("--", "");
	for(i=startVal; i <= endVal; i++){
		if(i < 10){
			i = "0" + i;  
		}
		ids.options[cnt] = new Option(i, i);
		if(i==selVal){
			ids.options[cnt].selected =true;
		}
		cnt++;
	}
}

function buildYearOption(ids, startVal, endVal, selVal){
	var ids = document.getElementById(ids);
	var cnt = 1;	
	startVal = parseInt(startVal);
	endVal = parseInt(endVal);
	selVal = parseInt(selVal);
	ids.options[0] = new Option("--", "");
	for(i=startVal; i >=(startVal-endVal); i--){
		if(i < 10){
			i = "0" + i;
		}
		ids.options[cnt] = new Option(i, i);
...
function buildMonthOption(ids, selVal){
	var ids = document.getElementById(ids);
	...
	for(i=0; i < monthArray.length; i++){
		ids.options[cnt] = new Option(monthArray[i], cnt1);
		if(cnt1==selVal){
			ids.options[cnt].selected = true;
		}
		cnt++;
...
function buildPriceDate(ids, startVal, endVal, selVal){
	var ids = document.getElementById(ids);
	var cnt = 1;
	startVal = parseInt(startVal);	
	endVal = parseInt(endVal);
	...
	for(i=startVal; i <= endVal; i++){
		...
		ids.options[cnt] = new Option(i, i);
		if(ids.options[cnt].value == selVal){
			//ids.options[cnt].selected = true;
			ids.options[cnt].setAttribute("SELECTED","SELECTED");
		}
		cnt++;
	}
}

function buildPriceMonth(ids, selVal){
	var ids = document.getElementById(ids);
	var monthArray = new Array();
	monthArray[0] = "Jan";
	monthArray[1] = "Feb";
...
function buildPriceYear(ids, startVal, endVal, selVal){
	var ids = document.getElementById(ids);
	var cnt = 1;
	startVal = parseInt(startVal);
	endVal = parseInt(endVal);
