...
class sitemap {
    ...
    private $version ='1.0.0'; // the version of the class
 ...
 */
	public function addElements($elementArray){
		...
		for($i=0;$i<$count;$i++){
			...
			$data->addChild('loc',htmlentities(strtolower($elementArray[$i]['loc'])));
 ...
 */
	private function fetch_remote_file ($url, $proxyHost= NULL , $proxyPort = NULL,$headers = "" ) {
		require_once(SITE_CLASSES_PATH.'Snoopy.class.php');
		$client = new Snoopy();
