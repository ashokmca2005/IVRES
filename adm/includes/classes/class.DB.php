	...
	function fun_db_connect(){
		...
		do {
			if($this->dbConnectPersistant){
				$conn = @mysql_pconnect($this->dbHost, $this->dbUsername, $this->dbPassword);
			}else{
				$conn = @mysql_connect($this->dbHost, $this->dbUsername, $this->dbPassword);
			}
			$tries--;
		...
		$this->doSql("set session wait_timeout=28800");
	}
...
*/
	...
	*/
	function getIdentity() {
		$identity = mysql_insert_id($this->dbLink);
		return $identity;
	}
...
# Function Gets last inserted id
	...
	* Reads all records into an array of hashes
	* @param	$rs	the recordset
	* @returns 	an array of arrays containing assoc the records
	*/
