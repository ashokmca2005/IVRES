...
if($_POST['securityKey']==md5(RESOURCEDELETE)){
	...
	echo "<script> location.href='admin-site-variables.php?sec=resource';</script>";
}
...
<form name="frmAddResource" id="frmAddResource" action="admin-site-variables.php?sec=resource&rsid=<?php echo $resourceInfoArr['resource_id']; ?>" method="post">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="font12">
    ...
    <tr>
        <td valign="top"><a href="admin-site-variables.php?sec=resource" class="back">Back to List</a></td>
                                            ...
                                            <td colspan="2" align="right" valign="top" class="header">
                                                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                    <tr>
														...
														?>
															<a href="javascript:showField('txtshwRjctTblId');" style="text-decoration:none;"><img src="images/rejectN.png" alt="Delete" width="71" height="21" border="0" /></a>&nbsp;<a href="javascript:sbmtResourceApproval(<?php echo $resource_id; ?>);" style="text-decoration:none;"><img src="images/approveN.png" alt="Approve" width="71" height="21" border="0" /></a>&nbsp;<a href="javascript:sbmtEvntDelete();" style="text-decoration:none;"><img src="images/DeleteN.png" alt="Delete" width="60" height="21" /></a>
														<?php
															} else {
														?>
                                            ...
                                            <?php
                                                if(isset($resourceInfoArr['resource_area_id']) && ($resourceInfoArr['resource_area_id'] != "" || $resourceInfoArr['resource_area_id'] != "0")) {
                                                    ...
                                                    <select name="txtArea" id="txtAreaId" onchange="return chkSelectArea();" style="display:block;" class="select216">
                                                        <?php 
                                                            $locationObj->fun_getAreaListOptions($resource_area_id, '84');
                                                        ?>
                                                    </select>
                                                    <?php
	...
	if(isset($_GET['sortby']) && $_GET['sortby'] != ""){
		...
		switch($_GET['sortby']){
			...
			case 'resourcecat':
				...
				$strQuery .= " A.resource_cat_ids ".$dr;
			break;
			case 'evnttitle':
				if($_GET['dr'] == "1"){
					$dr = "ASC";
				...
				$strQuery .= " A.resource_name ".$dr;
			break;
			case 'resourceaddeddate':
				if($_GET['dr'] == "1"){
					$dr = "ASC";
					$resourcecodedr 		= 1;
					$resourcecatdr 			= 1;
					$resourcetitledr		= 1;
					$resourceaddeddatedr 	= 0;
					$resourcestatusdr 		= 1;
				}
				else{
					$dr = "DESC";
...
//echo $strQuery;
	$resourceListArr = $resObj->fun_getPendingApprovalResourcesArr($strQuery);
//	print_r($resourceListArr);
	if(count($resourceListArr) > 0){
	?>
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				<td colspan="2" valign="top">
					<table width="100%" border="0" cellpadding="0" cellspacing="0" class="EventListing">
						<thead>
							<tr>
								<th width="10%" scope="col" onmouseover="this.className = 'RollOver';" onclick="sortList('<?php echo "admin-site-variables.php?sec=resource&sortby=resourcecode&dr=".$resourcecodedr;?>');" <?php if(isset($_GET['sortby']) && ($_GET['sortby'] == "resourcecode")){echo "class=\"current\" onmouseout=\"this.className = 'current';\" ";}else{echo "onmouseout=\"this.className = 'RollOut';\"";}?> ><div>ID</div></th>
								<th width="25%" scope="col" onmouseover="this.className = 'RollOver';" onclick="sortList('<?php echo "admin-site-variables.php?sec=resource&sortby=resourcecat&dr=".$resourcetitledr;?>');" <?php if(isset($_GET['sortby']) && ($_GET['sortby'] == "resourcecat")){echo "class=\"current\" onmouseout=\"this.className = 'current';\" ";}else{echo "onmouseout=\"this.className = 'RollOut';\"";}?> ><div>Category</div></th>
								<th width="35%" scope="col" onmouseover="this.className = 'RollOver';" onclick="sortList('<?php echo "admin-site-variables.php?sec=resource&sortby=evnttitle&dr=".$resourcecatdr;?>');" <?php if(isset($_GET['sortby']) && ($_GET['sortby'] == "evnttitle")){echo "class=\"current\" onmouseout=\"this.className = 'current';\" ";}else{echo "onmouseout=\"this.className = 'RollOut';\"";}?>><div>Title</div></th>
								<th width="15%" scope="col" onmouseover="this.className = 'RollOver';" onclick="sortList('<?php echo "admin-site-variables.php?sec=resource&sortby=resourceaddeddate&dr=".$resourceaddeddatedr;?>');" <?php if(isset($_GET['sortby']) && ($_GET['sortby'] == "resourceaddeddate")){echo "class=\"current\" onmouseout=\"this.className = 'current';\" ";}else{echo "onmouseout=\"this.className = 'RollOut';\"";}?>><div>Added Date</div></th>
								<th width="15%" scope="col" class="right" onmouseover="this.className = 'rightOver';" onclick="sortList('<?php echo "admin-site-variables.php?sec=resource&sortby=resourcestatus&dr=".$resourcestatusdr;?>');" <?php if(isset($_GET['sortby']) && ($_GET['sortby'] == "resourcestatus")){echo "class=\"rightOver\" onmouseout=\"this.className = 'rightOver';\" ";}else{echo "onmouseout=\"this.className = 'right';\"";}?>><div>Status</div></th>
							</tr>
						</thead>
						<tbody>
							<?php
                            for($i=0; $i < count($resourceListArr); $i++){
                            $resource_id 			= $resourceListArr[$i]['resource_id'];
                            $resource_cat_ids 		= $resourceListArr[$i]['resource_cat_ids'];
                            $resource_name 			= $resourceListArr[$i]['resource_name'];
//                            $resource_description 	= $resourceListArr[$i]['resource_description'];
                            $status_name 			= $resourceListArr[$i]['status_name'];
							$created_on 			= date('M d, Y', $resourceListArr[$i]['created_on']);
                            $active 				= $resourceListArr[$i]['active'];
                            ?>
                                <tr>
                                    <td class="left"><a href="admin-site-variables.php?sec=resource&rsid=<?php echo $resource_id;?>"><?php echo fill_zero_left($resource_id, "0", (6-strlen($resource_id)));?></a></td>
                                    <td><?php echo $resObj->fun_getResourceCatName($resource_cat_ids); ?></td>
	...
	<?php
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td valign="top">No Resources Added!</td>
			</tr>
		</table>
		<?php
	}
