...
<tr><td>&nbsp;</td></tr>
<tr><td>&nbsp;Hi,</td></tr>
<tr><td><img src="'.SITE_URL.'images/spacer.gif" width="20" height="1"> '.trim(ucfirst($owner_fname." ".$owner_lname)).' </td></tr>
...
<?php
	...
	$propPospondListArr = $propertyObj->fun_getPendingApprovalSuspendedPropertyArr($strQuery);
	if(is_array($propPospondListArr) && count($propPospondListArr) > 0){
	?>
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td valign="top">Display <?php echo count($propPospondListArr); ?>-<?php echo count($propPospondListArr); ?> of <?php echo count($propPospondListArr); ?></td>
			...
			<tr>
				<td colspan="2" valign="top">
					<table width="100%" border="0" cellpadding="0" cellspacing="0" class="EventListing">
						...
						<tbody>
							<?php
                            for($i=0; $i < count($propPospondListArr); $i++){
                            $prop_id 			= $propPospondListArr[$i]['property_id'];
                            $prop_name 			= $propPospondListArr[$i]['property_name'];
                            $prop_updated_on 	= date('M d, Y', strtotime($propPospondListArr[$i]['updated_on']));
                            $prop_created_on 	= date('M d, Y', strtotime($propPospondListArr[$i]['created_on']));
                            $prop_active_on 	= date('M d, Y', strtotime($propPospondListArr[$i]['active_on']));
							$prop_expire_on 	= date("M d, Y", mktime(0,0,0, date("m", strtotime($propPospondListArr[$i]['active_on'])), date("d", strtotime($propPospondListArr[$i]['active_on'])),date("Y", strtotime($propPospondListArr[$i]['active_on']))+1));

                            $prop_ownr_id 		= $propPospondListArr[$i]['owner_id'];
                            $prop_status 		= ucwords($propPospondListArr[$i]['status_name']);
                            $prop_ownr_name 	= ucwords($propPospondListArr[$i]['user_fname']." ".$propPospondListArr[$i]['user_lname']);
                            ?>
                                <tr>
	...
	?>
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td valign="top">No suspended property found!</td>
			</tr>
		</table>
	<?php
	}
