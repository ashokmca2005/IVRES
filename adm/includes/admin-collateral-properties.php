		...
		if($errorMsg == 'no' && $errorMsg != 'yes'){
			if($propertyObj->fun_editProperty($property_id) === true){
				...
				if(isset($_POST['txtPriceId']) && $_POST['txtPriceId'] != "") {
					echo "<script>window.location='admin-collateral.php?sec=".$_GET['sec']."&subsec=pri&pid=".$property_id."&msg=updatesuccess'; </script>";
				} else {
					echo "<script>window.location='admin-collateral.php?sec=".$_GET['sec']."&subsec=pri&pid=".$property_id."&msg=addsuccess'; </script>";
				}
			} else {
				echo "<script> location.href = window.location; </script>";
			}
		...
		if($_POST['addAvailablity'] == '1'){
			...
			if($errorMsg == 'no' && $errorMsg != 'yes'){
				...
				if($propertyObj->fun_addPropertyAvailablityDetails($property_id, $txtStartDate, $txtEndDate, $txtStatus) === true){
					$propertyObj->fun_updatePropertyLastUpdate($property_id);
					echo "<script>window.location='admin-collateral.php?sec=".$_GET['sec']."&subsec=ava&pid=".$property_id."&msg=updatesuccess'; </script>";
				} else {
					echo "<script> location.href = window.location; </script>";
				}
			} else {
...
$strMessage .= '<tr><td>&nbsp;Best Regards,</td></tr>
	...
	$linkdet = "admin-collateral.php?sec=".$sec."&subsec=det".$strUrlAdd;
	$linkpho = "admin-collateral.php?sec=".$sec."&subsec=pho".$strUrlAdd;
	$linkloc = "admin-collateral.php?sec=".$sec."&subsec=loc".$strUrlAdd;
	$linkpri = "admin-collateral.php?sec=".$sec."&subsec=pri".$strUrlAdd;
	$linkava = "admin-collateral.php?sec=".$sec."&subsec=ava".$strUrlAdd;
	$linkcon = "admin-collateral.php?sec=".$sec."&subsec=con".$strUrlAdd;
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td valign="top"><a href="admin-collateral.php?sec=<?php echo $sec;?>" class="arrowLinkback">Back to list</a></td>
        ...
        <td colspan="2" valign="top">
            <table border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    ...
                    <td valign="top" width="80"><strong>Checklist</strong></td>
                    <td valign="top" width="180"><?php if(isset($propUpdateInfoArr['checklist_updated_on']) && $propUpdateInfoArr['checklist_updated_on']!="") {echo date('M d, Y', strtotime($propUpdateInfoArr['checklist_updated_on'])); if((int)((mktime(0, 0, 0, date('m'), date('d'), date('Y')) - strtotime($propUpdateInfoArr['checklist_updated_on'])) / (60 * 60 * 24)) < 7) { echo "<img src=\"".SITE_ADMIN_IMAGES."Flag.gif\" class=\"Flag\" alt=\"Flag\" width=\"9\" height=\"15\" />"; }} else { echo " ----- ";} ?></td>
                </tr>
                <tr>
                    <td valign="top" width="100">&nbsp;</td>
                    ...
                    <td valign="top" width="80"><strong>Specials</strong></td>
                    <td valign="top" width="180"><?php if(isset($propUpdateInfoArr['deal_updated_on']) && $propUpdateInfoArr['deal_updated_on']!="") {echo date('M d, Y', strtotime($propUpdateInfoArr['deal_updated_on']));  if((int)((mktime(0, 0, 0, date('m'), date('d'), date('Y')) - strtotime($propUpdateInfoArr['deal_updated_on'])) / (60 * 60 * 24)) < 7) { echo "<img src=\"".SITE_ADMIN_IMAGES."Flag.gif\" class=\"Flag\" alt=\"Flag\" width=\"9\" height=\"15\" />"; }} else { echo " ----- ";} ?></td>
                </tr>
                <tr>
                    <td valign="top" width="100">&nbsp;</td>
							...
							<table cellpadding="0" cellspacing="0" width="100%">
								<tr>
									...
									<ul>
										...
										?>
										<li style="padding-top:10px; float:right; padding-right:5px;"><a href="#" class="blue-link">Preview</a> | <a href="#" onClick="MM_openWindow('<?php echo $helpPage; ?>', 'childwindow', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=no,width=700,height=800')" class="blue-link">Help?</a></li>
									</ul>
									<!--Tabbing Ends Here -->
									</td>
								</tr>
...
<?php
	...
	if(isset($_POST['txtFilter']) && $_POST['txtFilter'] == "1"){		
		...
		$strQuery = " AND A.property_id='".$txtProperty."' ";
	}
...
//		$strQuery 	= "";
	...
	$propListArr = $propertyObj->fun_getCollateralPropertyArr($strQuery);
	//print_r($propListArr);
	...
	<script language="javascript" type="text/javascript">
		...
		var x, y;
		function getFilter(){
			var id = document.getElementById("txtPropertyId").value;
			if(id == "") {
				alert("Enter property id!");
				document.getElementById("txtPropertyId").focus();
				return false;
			} else {
				document.getElementById("frmFilter").action = "admin-collateral.php?sec=prop";
		...
		function toggleLayer(whichLayer){
            ...
            } else if(whichLayer == 'property-delete-pop') {		
                var x1 = x-150;
                var y1 = y-100;
                output = '<div style="z-index:5;">'+output+'</div>';
		...
		function delPropertyItem(){
			...
			if(document.getElementById("txtDelItem").value != "") {
				...
				req.onreadystatechange = handleDeletePropertyItemResponse;
				req.open('get', 'includes/ajax/propertydeleteXml.php?prop_id='+strPropertyId); 
				req.send(null);   
			}
		}
		function handleDeletePropertyItemResponse(){
			if(req.readyState == 4){
				var response = req.responseText;
				xmlDoc = req.responseXML;
				var root = xmlDoc.getElementsByTagName('properties')[0];
				if(root != null){
					...
					for (var i = 0 ; i < items.length ; i++){
						var item 				= items[i];
						var propertystatus 		= item.getElementsByTagName("propertystatus")[0].firstChild.nodeValue;
						if(propertystatus == "property deleted."){
							window.location = location.href;
						}
					}
				}
			...
			function showFilter(strInt){
				if(parseInt(strInt) > 0) {
					...
					location.href = "admin-collateral.php?sec=prop";
				}
            }
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						<tr>
							...
							<td align="right" valign="top">
								<table border="0" cellspacing="0" cellpadding="0" id="filterTbl" style="display:<?php if(isset($_POST['txtFilter']) && $_POST['txtFilter'] == "1") { echo "block";} else {echo "none";}?>">
									<tr>
										<td class="blackTxt14 pad-rgt5" style="font-weight:normal;">Property ID</td>
										<td class="pad-rgt5"><input type="text" name="txtProperty" id="txtPropertyId" class="Textfield80 blackText" value="<?php if(isset($_POST['txtProperty']) && $_POST['txtProperty'] != "") { echo $_POST['txtProperty'];} else {echo "";}?>" /></td>
			...
			<tr>
                    ...
                    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="EventListing">
						<thead>
							<tr>
								...
								<th width="2%" scope="col" onmouseover="this.className = 'RollOver';" onclick="sortList('<?php echo "admin-collateral.php?sec=".$sec."&sortby=propid&dr=".$propiddr;?>');" <?php if(isset($_GET['sortby']) && ($_GET['sortby'] == "propid")){echo "class=\"current\" onmouseout=\"this.className = 'current';\" ";}else{echo "onmouseout=\"this.className = 'RollOut';\"";}?> ><div>Prop ID</div></th>
								<th width="2%" scope="col" onmouseover="this.className = 'RollOver';" onclick="sortList('<?php echo "admin-collateral.php?sec=".$sec."&sortby=ownrid&dr=".$ownriddr;?>');" <?php if(isset($_GET['sortby']) && ($_GET['sortby'] == "ownrid")){echo "class=\"current\" onmouseout=\"this.className = 'current';\" ";}else{echo "onmouseout=\"this.className = 'RollOut';\"";}?> ><div>Owner ID</div></th>
								<th width="10%" scope="col" onmouseover="this.className = 'RollOver';" onclick="sortList('<?php echo "admin-collateral.php?sec=".$sec."&sortby=ownrname&dr=".$ownrnamedr;?>');" <?php if(isset($_GET['sortby']) && ($_GET['sortby'] == "ownrname")){echo "class=\"current\" onmouseout=\"this.className = 'current';\" ";}else{echo "onmouseout=\"this.className = 'RollOut';\"";}?>><div>Owner name</div></th>
								<th width="22%" scope="col" onmouseover="this.className = 'RollOver';" onclick="sortList('<?php echo "admin-collateral.php?sec=".$sec."&sortby=propname&dr=".$propnamedr;?>');" <?php if(isset($_GET['sortby']) && ($_GET['sortby'] == "propname")){echo "class=\"current\" onmouseout=\"this.className = 'current';\" ";}else{echo "onmouseout=\"this.className = 'RollOut';\"";}?>><div>Property name</div></th>
                                <th width="19%" scope="col" onmouseover="this.className = 'RollOver';" onclick="sortList('<?php echo "admin-collateral.php?sec=".$sec."&sortby=register&dr=".$updatedr;?>');" <?php if(isset($_GET['sortby']) && ($_GET['sortby'] == "update")){echo "class=\"current\" onmouseout=\"this.className = 'current';\" ";}else{echo "onmouseout=\"this.className = 'RollOut';\"";}?>><div>Registration date</div></th>
								...
								<th width="5%" scope="col" onmouseover="this.className = 'RollOver';" onclick="sortList('<?php echo "admin-collateral.php?sec=".$sec."&sortby=update&dr=".$updatedr;?>');" <?php if(isset($_GET['sortby']) && ($_GET['sortby'] == "update")){echo "class=\"current\" onmouseout=\"this.className = 'current';\" ";}else{echo "onmouseout=\"this.className = 'RollOut';\"";}?>><div>Paid</div></th>
								<th width="5%" scope="col" onmouseover="this.className = 'RollOver';" onclick="sortList('<?php echo "admin-collateral.php?sec=".$sec."&sortby=status&dr=".$statusdr;?>');" <?php if(isset($_GET['sortby']) && ($_GET['sortby'] == "status")){echo "class=\"rightOver\" onmouseout=\"this.className = 'rightOver';\" ";}else{echo "onmouseout=\"this.className = 'right';\"";}?>><div>Status</div></th>
						...
						<tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="right"><a href="javascript:delItem(<?php echo $prop_id; ?>);toggleLayer('property-delete-pop');" class="removeText">Delete</a></td>
                                </tr>
                            <?php
                            }
                            ?>
				...
				<div id="property-delete-pop" style="display:none; position:absolute; background:transparent; left:300px; top:500px;">
                                                ...
                                                <tr>
                                                    <td  align="left" valign="top" class="PopTxt">
                                                        <table width="98%" border="0" cellpadding="0" cellspacing="0">
                                                            <tr>
                                                                <td class="pad-rgt10 pad-top5"><strong>You will be delete the Property & information related to this Property !</strong></td>
                                                            </tr>
                                                            <tr>
                                                                <td class="pad-top10">
                                                                    <div class="FloatLft"><a href="javascript:closeWindow();void(0);" ><img src="<?php echo SITE_IMAGES;?>cancel-admin.png" alt="Keep it" /></a></div>
                                                                    <div class="FloatLft pad-lft5"><a href="javascript:delPropertyItem();"><img src="<?php echo SITE_IMAGES;?>delete.gif" alt="Delete it" onmouseover="this.src='<?php echo SITE_IMAGES; ?>delete_h.gif'" onmouseout="this.src='<?php echo SITE_IMAGES; ?>delete.gif'" /></a></div>
                                                                </td>
                                                            </tr>
                                                        </table>
                                                    </td>
	...
	?>
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td valign="top">No property added!</td>
			</tr>
		</table>
	<?php
	}
