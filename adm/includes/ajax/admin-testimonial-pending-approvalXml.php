...
if(isset($_GET['testid']) && $_GET['testid'] !=""){
	...
	switch($mode){
		case 'approve':
			$status 		= 2;
			$active 		= 1;
			$strUpdateQuery = "UPDATE ".TABLE_TESTIMONIALS." SET status='$status', active_on='$cur_unixtime', active_by='$cur_user_id', active='$active' WHERE testimonial_id='".$testimonial_id."'";
